...
void createDecodeTable(decodeTable *table, int* symbol_spread) {
    ...
    int *next = (int *)malloc(table->L * sizeof(int));
    ...
    for(int X = 0; X < table->L; X ++){
        ...
        table->table[X].nb = (int)(R - floor(log2(x_tmp)));
...
decodeTable *initDecodeTable(int L, int *s_list, int *L_s, int n_sym) {
    ...
    int *sym_spread = fast_spread(table->L, table->L_s, table->n_sym, 0, (int)((5.0/8.0) * table->L + 3));
...
void decodeStep(decoder *d, decodeTable *table){
    // get the symbol
    int s_decode = table->table[d->state].sym;

    // read bits from bitstream
    int bits = readBits(d, table->table[d->state].nb);
...
decoder *decode(uint8_t *bitstream, long l_bitstream, decodeTable *table){
    ...
    while((d->l_bitstream != 0) || d->state != state_orig){
        ...
        d->l_msg ++;
        d->msg = (int *)realloc(d->msg, sizeof(int) * (d->l_msg + 1));
...
int main() {
    ...
    // int *s_list = (int *)malloc(sizeof(int) * 256);
    // for (int i = 0; i < 8; i++) {
    //     s_list[i] = i;
    // }
    // int L_s[8] = {49,39,59,49,29,23,4,4};

    // int bs[6] = {0b10110100, 0b11011111, 0b10010001, 0b11011000, 0b11100101, 0b00000000};
    ...
    int n_sym;
    fread(&n_sym, sizeof(int), 1, f);

    printf("Number of symbols: %d\n", n_sym);

    // get the symbols
    int *s_list = (int *)malloc(sizeof(int) * n_sym);
    fread(s_list, sizeof(int), n_sym, f);
    ...
    // read the bitstream
    int *bs = (int *)malloc(sizeof(int) * l_bs);
    fread(bs, sizeof(int), l_bs, f);
    fclose(f);
