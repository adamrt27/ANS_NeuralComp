...
void createEncodeTable(encodeTable *table, const int *sym_spread) {
    ...
    int *next = (int *)malloc(table->n_sym * sizeof(int));

    for (int i = 0; i < table->n_sym; i++) {
        table->k[i] = R - (int)floor(log2(table->L_s[i]));
...
encodeTable *initEncodeTable(int L, int *s_list, int *L_s, int n_sym) {
    ...
    table->k = (int *)malloc(sizeof(int) * table->n_sym);
...
int main(){
    ...
    int msg[18] = {0,3,2,3,2,1,2,3,4,7,3,1,2,3,4,5,6,7};
    ...
    fwrite(&res->n_sym, sizeof(int), 1, f); // write the number of symbols
    // writing symbols
    fwrite(s_list, sizeof(int), res->n_sym, f); // write the symbols
    // writing L_s
    fwrite(L_s, sizeof(int), res->n_sym, f); // write the L_s
    // writing the bitstream
    fwrite(e->bitstream, sizeof(int), e->l_bitstream, f);
    fclose(f);
